...
Background thread management for non-blocking analysis:
- **AnalysisWorker**: Translates scientific logic to Qt signals.
- **TaskScheduler**: Centralized singleton that manages a global thread pool.
- Prevents thread exhaustion and simplifies plugin development.
- Enables responsive UI during long operations without manual `QThread` management.
...
- **Centralized task scheduling** (no manual QThread management)
...
**Phase 3 (Done)**: Concurrency & Performance
- ✅ Centralized TaskScheduler singleton
- ✅ Global QThreadPool management
- ✅ Resource fairness and thread safety
- 🔲 Image processing helpers (contrib)
- 🔲 Plotting utilities
...
A: Use `task_scheduler.submit(analyzer, state)`. It handles all threading for you and avoids resource starvation.
