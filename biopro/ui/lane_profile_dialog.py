...
class LaneProfileDialog(QDialog):
    ...
    def _on_mouse_move(self, event) -> None:
        if getattr(self, "combo_lane", None) is None:
            return
        # Track dragging so release handler can distinguish click from drag
    ...
    def _on_mouse_release(self, event) -> None:
        """Emit click signal only when the mouse was not dragging."""
        if not event.inaxes:
            self._drag_active = False
            return
        ...
        # If a span drag was in progress, SpanSelector handles it — skip click
        if self._drag_active:
            self._drag_active = False
            return

        if event.button == 1:
            # Suppress while toolbar pan/zoom is active
            try:
    ...
    def _on_span_select(self, xmin: float, xmax: float) -> None:
        """Handle drag-region selection via SpanSelector."""
        self._drag_active = True
        idx = self.combo_lane.currentIndex()
        if idx < 0:
            return
        # x-axis is inverted so normalize to pixel order
