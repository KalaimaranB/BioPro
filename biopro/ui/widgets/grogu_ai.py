...
class GroguAI(QWidget):
    ...
    def _draw_robe_body(self, p):
        ...
        p.setPen(
            QPen(self.robe_shadow.darker(140), 5, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
    ...
    def _draw_collar(self, p):
        ...
        p.setPen(
            QPen(self.robe_shadow.darker(110), 6, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
        fold1 = QPainterPath()
        fold1.moveTo(-90, 25)
    ...
    def _draw_eyes(self, p):
        ...
        for side in [-1, 1]:
            ...
            socket.setColorAt(
                0.6, QColor(self.skin_deep.red(), self.skin_deep.green(), self.skin_deep.blue(), 40)
            )
            ...
            mask.cubicTo(
                -eye_w * 0.2, top_y - 8, eye_w * 0.3, top_y + 10, eye_w * 0.48, eye_h * 0.15
            )

            # Lower lid: Very round bowl shape
            mask.cubicTo(
                eye_w * 0.4, eye_h * 0.55, -eye_w * 0.4, eye_h * 0.5, -eye_w * 0.48, -eye_h * 0.05
            )
            mask.closeSubpath()
            ...
            p.drawChord(
                QRectF(-eye_w * 0.4, -eye_h * 0.45, eye_w * 0.8, eye_h * 0.6), 30 * 16, 120 * 16
            )
            ...
            p.setPen(
                QPen(
                    self.skin_deep.darker(115), 3.5, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap
                )
            )
            crease = QPainterPath()
            crease.moveTo(-eye_w * 0.5, -eye_h * 0.05)
            crease.cubicTo(
                -eye_w * 0.2, top_y - 18, eye_w * 0.3, top_y + 5, eye_w * 0.5 + 5, eye_h * 0.2
            )
            p.drawPath(crease)
            ...
            lower_lid.cubicTo(
                -eye_w * 0.1, eye_h * 0.6, eye_w * 0.3, eye_h * 0.6, eye_w * 0.45, eye_h * 0.3
            )
    ...
    def _draw_face_details(self, p):
        ...
        def draw_soft_crease(x, y, w, h, alpha=60):
            ...
            grad.setColorAt(
                0.0,
                QColor(self.skin_deep.red(), self.skin_deep.green(), self.skin_deep.blue(), alpha),
            )
        ...
        p.setPen(
            QPen(self.skin_deep.darker(160), 2, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
        # Nose holes
        ...
        mouth_y = 32
        p.setPen(
            QPen(self.skin_deep.darker(140), 2, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
    ...
    def _draw_peach_fuzz(self, p):
        p.setPen(
            QPen(QColor(255, 255, 255, 70), 0.8, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
        ...
        for _ in range(50):
            ...
            if hx < 0:
                angle = math.radians(rand.uniform(210, 260))
            if hx > 0:
                angle = math.radians(rand.uniform(280, 330))
