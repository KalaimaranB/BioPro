...
class AssetManager:
    ...
    def validate_assets(self, data: dict[str, Any]) -> bool:
        ...
        for file_hash, asset in data["assets"].items():
            has_local = False
            if asset.get("local_path") and (self.project_dir / asset["local_path"]).exists():
                has_local = True

            has_original = False
            if asset.get("original_path") and Path(asset["original_path"]).exists():
                has_original = True
    ...
    def get_asset_path(self, data: dict[str, Any], file_hash: str) -> Path | None:
